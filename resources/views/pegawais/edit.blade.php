    ...
    <div class="container mt-5">
        ...
        <form action="{{ route('pegawais.update', $pegawai->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama" class="form-control" id="nama" value="{{ old('nama', $pegawai->nama) }}" placeholder="Nama Pegawai">
            </div>

            <div class="mb-3">
                <label for="email" class="form-label">Email Pegawai</label>
                <input type="email" name="email" class="form-control" id="email" value="{{ old('email', $pegawai->email) }}" placeholder="Email Pegawai">
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="kantor_id" class="form-select" id="kantor_id">
                    ...
                    @foreach($kantors as $kantor)
                        <option value="{{ $kantor->id }}" {{ (old('kantor_id', $pegawai->kantor_id) == $kantor->id) ? 'selected' : '' }}>
                            {{ $kantor->nama }}
                        </option>
                    @endforeach
                </select>
